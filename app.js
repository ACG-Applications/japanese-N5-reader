...
class App {
  ...
  init() {
    ...
    console.log("✅ appData found:", window.appData.blocks?.length || 0, "blocks");
    ...
    console.log(`📖 ${window.appData?.grammarPatterns?.length || 0} grammar patterns available`);
  }
  ...
  updateTTSButtons(isPlaying) {
    if (this.elements.ttsPlayBtn) {
      this.elements.ttsPlayBtn.textContent = isPlaying ? "⏹️ Playing..." : "🔊 Play Story";
      if (isPlaying) {
  ...
  playStoryTTS() {
    ...
    utterance.onboundary = (event) => {
      if (event.name === "sentence" || event.name === "word") {
        const progress = Math.min((event.charIndex / fullText.length) * 100, 100);
        if (this.elements.ttsProgress) {
  ...
  loadBlock(blockId) {
    ...
    this.currentBlockData = window.appData?.blocks?.find((b) => b.id === blockId);
  ...
  renderSentence(sentence, index) {
    ...
    if (this.settings.showGrammar && sentence.patternIds && sentence.patternIds.length > 0) {
      const patternClasses = sentence.patternIds.map((id) => `pattern-${id}`).join(" ");
        ...
        ${this.settings.showGrammar && sentence.patternIds
          ...
          <div class="sentence-patterns">
            <small>Patterns: ${sentence.patternIds
              .map((id) => {
                const pattern = window.appData?.grammarPatterns?.find((p) => p.id === id);
  ...
  submitQuizAnswer() {
    ...
    this.showFeedback(
      isCorrect ? "🎉 Correct!" : `❌ Incorrect. The answer is: ${question.correct}`,
  ...
  renderGrammar() {
    ...
    const grammarHTML = grammarFocus
      .map((item) => {
        const pattern = window.appData?.grammarPatterns?.find((p) => p.id === item.patternId);
        const mastery = this.analytics ? this.analytics.getPatternMastery(item.patternId) : null;
        const showFurigana = this.settings.showFurigana;
  ...
  setupStickyControls() {
    const controlsBar = this.elements.controlsBar || document.querySelector('.controls-bar');
